...
    e.preventDefault(); // ✅ หยุดการ reload หน้าเว็บเพื่อให้ required ทำงาน
    ...
    // ✅ ตรวจสอบชื่อผู้ใช้: ต้องมีเว้นวรรค 1 ครั้งเท่านั้น
    const cleanName = name.trim();
    const nameParts = cleanName.split(' ');

    if (nameParts.length !== 2) {
        alert("❌ รูปแบบชื่อไม่ถูกต้อง \nกรุณากรอก 'ชื่อ เว้นวรรค นามสกุล' \n(ต้องมีเว้นวรรค 1 ครั้งเท่านั้น และห้ามมีช่องว่างเกิน)");
        return; 
    }
...
    e.preventDefault(); 
    ...
    // ✅ ตรวจสอบชื่อนักโทษ: ต้องมีเว้นวรรค 1 ครั้งเท่านั้น
    const cleanName = namePrisoner.trim();
    const nameParts = cleanName.split(' ');
...
      } catch (err) {
        if(err.response){
          alert("เพิ่มผู้ต้องขังไม่สำเร็จ" + (err.response?.data?.message || "เกิดข้อผิดพลาด" ))
        }
      }
    }
  
...
                <input  
                    className="form-control"
                    placeholder="ชื่อ-สกุล (ต้องมีเว้นวรรค 1 ครั้ง)"
                    value={namePrisoner}
...
      <button className="btn-secondary" onClick={() => navigate('/printpage')}>พิมพ์หนังสือ(PDF)</button>
     <button className="btn-secondary" onClick={() => navigate('/printpageonline')}>พิมพ์หนังสือ ออนไลน์(PDF)</button>
